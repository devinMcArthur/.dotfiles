...
- 2026-06-02: virt-viewer --full-screen is unreliable on Wayland/Hyprland with multiple differently-sized monitors: it sizes the SPICE guest framebuffer to the PRIMARY monitor's geometry (e.g. eDP-1 1920x1280 logical) regardless of which monitor the compositor's window rule actually places it on (external KA272 1920x1080), because Hyprland positions the window AFTER virt-viewer has chosen the geometry. Symptom: guest renders too tall → ~100px black bar at top, Windows taskbar clipped off bottom; appears only when docked/plugged-in. Stripping the persisted per-VM monitor-mapping in ~/.config/virt-viewer/settings does NOT fix it (wrong-monitor geometry is chosen independently). FIX: run virt-viewer WINDOWED and let Hyprland own fullscreen so guest auto-resize follows the real window size and re-sizes across plug/unplug. Can't use a static `windowrule = fullscreen` (Hyprland 0.55 parser rejects bare action words) — dispatch `hyprctl dispatch fullscreen 0` from the launcher script after the window maps. Tradeoff: windowed virt-viewer drops the keyboard-shortcuts-inhibit grab, so Super combos go to Hyprland (Alt+Tab/ordinary keys still reach the guest when focused). Implemented in ~/.local/bin/win (chezmoi dot_local/bin/executable_win).
- 2026-05-28: Wayland clipboard offers die when the setting app exits — that's the protocol, not a bug. swappy's Copy button "silently failing" and `wl-copy < file` not surviving past the script that ran it are both this same root cause. cliphist STORES the bytes for history but doesn't own a live offer. Fix: `wl-clip-persist --clipboard regular` (Arch package wl-clip-persist) as a Hyprland exec-once. It re-claims any offer as its own and keeps it alive indefinitely. Single fix for screenshot Copy, drag-and-drop, any app that briefly touches the clipboard then exits.
- 2026-05-28: Chromium on Hyprland is unreliable in 2026-05: native --ozone-platform=wayland renders blank-white surface even with --disable-features=Vulkan + --disable-gpu, and --ozone-platform=x11 (XWayland) was also broken for SPA dashboards in testing. Default to Firefox (native Wayland in Chromium-free codebases is fine) for browser-default purposes on this Hyprland setup. Save Chromium investigation cycles for when there's a specific Chrome-only requirement.
- 2026-05-28: Vanilla Arch's /usr/bin/chromium is a stripped ELF binary, NOT a shell wrapper — ~/.config/chromium-flags.conf is silently ignored. That flag-file convention only exists on Manjaro/Ubuntu where they ship a wrapper script that sources it. To pass flags persistently on Arch, override the .desktop file (drop a copy at ~/.local/share/applications/chromium.desktop with flags baked into all Exec= lines — user-level .desktop wins over /usr/share/applications/).
- 2026-05-28: When a GUI app's "Copy to clipboard" button silently fails on Wayland (swappy 1.8.0 is a confirmed offender), don't waste time on swappy/GTK debugging — bypass the app's Copy entirely and have the wrapper script do `wl-copy --type image/png < file` after the app exits. cliphist's image watcher picks it up. Diagnostic shortcut: `cliphist list | head` shows whether any image entries are landing in history; if older entries exist but recent attempts produced none, the app's Copy is the broken link, not the clipboard stack.
